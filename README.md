...
Reproducible pipeline and data for the study *"Diversifying selection on the respiratory
syncytial virus G glycoprotein is confined to the mucin-like flanks, while the central
conserved region is under purifying constraint."*

**Summary of findings.** Using a uniform align → maximum-likelihood phylogeny → site-wise
selection (FEL) pipeline — validated on influenza H3N2 haemagglutinin, where positive selection
is significantly enriched in antigenic sites A–E (*p* = 0.0081) — we find **no enrichment** of
positively selected sites in the RSV G central conserved region (CCR) or CX3C motif in RSV-A,
RSV-B, or pooled data (all *p* = 1.0). A powered, threshold-free region-level test shows the CCR
is under significantly stronger purifying selection than the mucin-like domain in RSV-A
(Kruskal–Wallis *p* = 0.027; CCR vs mucin *p* = 0.0066), with a concordant trend in RSV-B. All
mappable positively selected sites fall in the C-terminal mucin-like domain. Structural mapping
onto the CX3C/cystine-noose core (PDB 5WN9) confirms the purifying signature.
...
├── results/         alignments, ML trees, FEL outputs, per-site/region tables (JSON/CSV)
...
(`results/*_fel.json`, `results/*_fel_sites.csv`), analysis scripts (`code/`), figures
...
| HyPhy (FEL) | 2.5.100 (Docker `quay.io/biocontainers/hyphy:2.5.100`) | site-wise selection |
...
selection (HyPhy FEL) are external steps; their inputs and outputs are in `data/` and `results/`.
...
6. **HyPhy FEL** — per-codon dN/dS (external).
...
13. `code/make_figures.py` — permutation-null histograms, site lollipop, trees, and summary tables.
14. `code/region_selection_figure.py` — per-codon dN−dS distribution figure.
15. `code/structure_figure.py` — Matplotlib structure figure; also writes the B-factor-encoded PDB.
16. `code/render_ccd_pymol.pml` (open-source PyMOL) + `code/compose_structure_figure.py` —
