# Diversifying selection on the respiratory syncytial virus G glycoprotein is confined to the mucin-like flanks, while the central conserved region is under purifying constraint
...
**Running title:** Selection is excluded from the RSV G conserved core

**Keywords:** respiratory syncytial virus; G glycoprotein; central conserved region; CX3C motif; purifying selection; dN/dS; molecular evolution; FEL
...
The attachment (G) glycoprotein of respiratory syncytial virus (RSV) is the principal target of the antibody response and the most variable protein in the RSV genome, yet it carries a short central conserved region (CCR, residues 157–198 in the RSV-A2 reference) that harbours a CX3C chemokine-mimic motif and a disulfide-bonded "cystine noose." We tested a pre-registered hypothesis that episodic positive selection is enriched in the CCR and/or the CX3C motif. Using a uniform align → maximum-likelihood phylogeny → site-wise selection (FEL) pipeline, validated against influenza H3N2 haemagglutinin (positive selection significantly enriched in antigenic sites A–E; *p* = 0.0081), we found **no enrichment** of positively selected sites in the RSV G CCR or CX3C motif in RSV-A, RSV-B, or the pooled data (all *p* = 1.0). Because that site-count test is underpowered (3–6 significant sites per subtype), we added a properly powered, threshold-free companion analysis comparing the per-codon dN−dS distribution across regions. In RSV-A the CCR is under significantly stronger purifying selection than the C-terminal mucin-like domain (Kruskal–Wallis *p* = 0.027; CCR vs mucin Mann–Whitney *p* = 0.0066, rank-biserial −0.26); in RSV-B the same direction is present but does not reach significance (*p* = 0.074). All positively selected, mappable sites (6/6 across both subtypes) fall in the C-terminal mucin-like hypervariable domain. Mapping per-codon dN−dS onto the experimental structure of the CX3C/cystine-noose core (PDB 5WN9) confirms a purifying-selection signature over the functional motif. We conclude that immune-driven diversification of RSV G is spatially excluded from the conserved core and concentrated in the mucin-like flanks — a pattern consistent with structural and functional constraint on the CX3C motif and with the CCR's promise as a stable vaccine and therapeutic target.
...
Here we discriminate between these hypotheses with a uniform, reproducible molecular-evolution pipeline. We **pre-registered** the primary hypothesis — enrichment of positively selected sites in the CCR and CX3C motif — and its region definitions (locked 2026-07-04, before the enrichment tests were run). We first validate the entire pipeline on influenza H3N2 haemagglutinin, where the expected answer (positive selection concentrated in antigenic sites) is known, and then apply it to RSV-A and RSV-B G. Recognising that the pre-registered site-count test is statistically weak when few sites reach significance, we complement it with a powered, threshold-free analysis of the selection-pressure distribution across regions, and we anchor the result to the experimental structure of the CX3C core.
...
Per-codon selection was estimated with **FEL (Fixed Effects Likelihood)** in **HyPhy v2.5.100** (run in Docker; image `quay.io/biocontainers/hyphy:2.5.100`). FEL estimates a synonymous rate (α = dS) and a non-synonymous rate (β = dN) at each codon. A site was called **positively (diversifying) selected** when β > α at *p* ≤ 0.05, and **purifying** when β < α at *p* ≤ 0.05. The per-codon selection metric used throughout is **dN − dS (β − α)**.
...
Python 3.11 with Biopython 1.87, NumPy, SciPy, and Matplotlib. All inputs, outputs, scripts, and a full lab notebook with SHA256 checksums are in the project repository. Every figure is regenerated from `results/*.json` and `results/*.csv` only; no values are hand-entered.
...
Applied to influenza H3N2 HA, the pipeline detected five positively selected codons, **all five (100%) of which fall in antigenic sites A–E** (observed 100% vs 39.4% expected by chance; enrichment 2.5×; permutation **_p_ = 0.0081**; Figure 1, Table 1). The individual sites map to HA1 antigenic sites C (residue 50), A (135), B (157), B (193), and E (261) (Table S1). This confirms that the align → tree → FEL → enrichment workflow detects immune-driven diversifying selection where it is known to exist.

### 3.2 Positive selection is not enriched in the RSV G CCR or CX3C motif (primary result)
In contrast to influenza, **no** positively selected site fell within the RSV G CCR or CX3C motif in either subtype or in the pooled data (Table 1, Figure 2):
...
Our central finding is that adaptive diversification of the RSV G glycoprotein is **spatially partitioned**: it is excluded from the central conserved region — including the CX3C chemokine-mimic motif and its cystine noose — and confined to the mucin-like flanking domains. The primary (pre-registered) test returns a clean negative, and the powered companion analysis explains *why* that negative arises: the CCR is under purifying constraint, so it cannot be a hotspot of positive selection. The two results are complementary rather than contradictory.

This pattern is mechanistically coherent. The CX3C motif mimics fractalkine and engages CX3CR1, a function that plausibly imposes strong purifying selection on the motif and the disulfide scaffold that presents it; a diversifying substitution here would risk abolishing receptor engagement and disrupting the noose. Immune escape, by contrast, is cheaply achieved in the mucin-like domains, which are long, O-glycosylated, structurally disordered, and tolerant of substitution, insertion (e.g., the ON1 24-amino-acid duplication), and deletion. The influenza HA positive control demonstrates that our pipeline readily detects immune-driven diversifying selection when the immunodominant region is *not* structurally constrained — underscoring that the RSV result reflects biology, not insensitivity.
...
**Implications.** The exclusion of diversifying selection from the CCR strengthens the rationale for CCR/CX3C-directed vaccines and monoclonal antibodies: the epitope is evolutionarily stable, reducing the risk of escape that plagues variable-domain targets. Surveillance for antigenic change in G, conversely, should focus on the mucin-like domains, where the adaptive signal actually resides.
...
6. **FEL scope.** FEL detects pervasive site-wise selection; episodic, branch-specific, or lineage-restricted selection (e.g., MEME/aBSREL) was not modelled and could refine the picture.

---

## 6. Conclusion

Diversifying selection on the RSV G glycoprotein is confined to the mucin-like flanks and excluded from the central conserved region, which is instead under purifying constraint (significant in RSV-A, trending in RSV-B). The CX3C chemokine-mimic motif and its cystine noose lie within this constrained, purifying core. These results argue that the RSV G conserved region is an evolutionarily stable target for vaccines and antibody therapeutics, and that antigenic surveillance should be directed at the variable mucin-like domains.
...
**Figure 2. Positively selected sites in RSV G avoid the conserved core.** (a) Permutation-null for the CCR in the pooled RSV data; observed enrichment = 0 (*p* = 1.0). (b) Lollipop plot of every positively selected site along the RSV-A2 G coordinate axis, with the CCR and CX3C motif shaded; all mappable sites fall in the C-terminal mucin-like domain. `figures/rsv_combined_ccr_null_hist.png`, `figures/rsv_selected_sites_lollipop.png`
...
**Table 1. Enrichment of positively selected sites (permutation test, 10,000 permutations, seed 42, one-sided).**
...
**Table S1.** Full list of positively selected sites (flu + RSV-A + RSV-B) with reference coordinates and β estimates: `results/summary_selected_sites.csv`.

---

## Data and code availability
All sequence accessions, alignments, trees, FEL outputs, analysis scripts, figures, and a complete lab notebook with SHA256 checksums are provided in the project repository (`U6_project/`). Structure coordinates are from the RCSB PDB (accession 5WN9).
